def encode_movie(m)
	...
	ffmpeg_cmd = "ffmpeg -framerate 30 -threads 2 -report -i \"#{movie_data_path}\\img_%09d_Default_000.tif\" -pix_fmt yuv420p \"#{out_file}.m4v\""
	print "#{ffmpeg_cmd}\n=>\n"
	`#{ffmpeg_cmd}`
...
begin
	...
	unless ARGV[1].nil?
		...
		if prompt =="Y" or prompt == "" or prompt =="y"
			...
			concatenate_cmd = "ffmpeg -f concat -safe 0 -i #{temp_file} -c copy \"#{concat_fname}\""
			`#{concatenate_cmd}`
